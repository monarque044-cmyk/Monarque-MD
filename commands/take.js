...
import { Sticker, StickerTypes } from 'waifus-sticker-maker'; // ✅ Import unique et propre
...
export default {
    ...
    async execute(monarque, m, args) {
        try {
            ...
            // 🔍 1. Vérification du message cité (quoted)
            ...
            const packname = info[0]?.trim() || "𝕄𝕠𝕟𝕒𝕣𝕢𝕦𝕖 𝕄𝔻";
            const author = info[1]?.trim() || "𝟚𝟚𝟟";
            ...
            // 🎭 4. Création du nouveau sticker (nommé "finalSticker" pour éviter "already declared")
            const finalSticker = new Sticker(buffer, {
                pack: packname,
                author: author,
                type: StickerTypes.FULL,
                categories: ['🤩', '👑'],
                id: 'monarque_' + Date.now(),
                quality: 75,
            });
            ...
            await monarque.sendMessage(m.key.remoteJid, { 
                text: "⚠️ *Erreur* : Impossible de modifier ce sticker. Assure-toi qu'il n'est pas animé." 
            }, { quoted: m });
        }
    }
};
